...
import { handleReviewPlan } from "./review-plan.js";
...
beforeEach(() => {
  ...
  mockReviewPlan.mockResolvedValue(makeWorkflowResult());
});
...
describe("handleReviewPlan", () => {
  describe("provider resolution", () => {
    it("should use all providers with env keys set when models param is omitted", async () => {
      await handleReviewPlan({ plan: "Test plan" });

      expect(mockReviewPlan).toHaveBeenCalledOnce();
      const input = mockReviewPlan.mock.calls[0]![0] as ReviewPlanInput;
    ...
    it("should filter to only requested model IDs", async () => {
      await handleReviewPlan({ plan: "Test plan", models: ["anthropic", "openai"] });
    ...
    it("should return error when no API keys are set", async () => {
    ...
    it("should return error when requested model has no API key", async () => {
      ...
      const result = await handleReviewPlan({ plan: "Test plan", models: ["anthropic", "google"] });
  ...
  describe("synthesizer resolution", () => {
    ...
    it("should use specified synthesizer when provided", async () => {
      await handleReviewPlan({ plan: "Test plan", synthesizer: "openai" });
    ...
    it("should return error when synthesizer has no API key", async () => {
      ...
      const result = await handleReviewPlan({ plan: "Test plan", synthesizer: "google" });
  ...
  describe("workflow execution", () => {
    it("should pass plan and context through to reviewPlan", async () => {
      await handleReviewPlan({ plan: "My deployment plan", context: "Using Node.js" });

      const input = mockReviewPlan.mock.calls[0]![0] as ReviewPlanInput;
      expect(input.plan).toBe("My deployment plan");
      expect(input.context).toBe("Using Node.js");
    });
    ...
    it("should handle unexpected exceptions gracefully", async () => {
      mockReviewPlan.mockRejectedValue(new Error("Unexpected crash"));
  ...
  describe("deduplication", () => {
    it("should deduplicate model IDs", async () => {
      await handleReviewPlan({ plan: "Test plan", models: ["anthropic", "anthropic", "openai"] });
